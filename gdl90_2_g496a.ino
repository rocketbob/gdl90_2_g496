...
const int wifiSelectPin = 2;
const int maxTargets = 8;
const int warnAltitude = 5;
const byte warnDistance = 3;
...
void sendTIS() {
	...
	if (tNumberOf == 0) {
		...
		for (int t = 0; t < tNumberOf; t++) {
			...
			if (tByteDistance[t] <= warnDistance * 8 && tHundredsAltitude[t] <= warnAltitude) { //Set traffic warning if within warn dist and altitude
				writeByte(0x01);
			}
			else {
